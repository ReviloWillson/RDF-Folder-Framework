...
def rdf_create():
    ...
    if ".rdf" in rdfpth:
        ...
        rdf = False
        mkdirconf = input("Will attempt to mkdir; type [y] to proceed or press any other key to cancel.")
        mkdirconf = mkdirconf.lower()
        if "y" in mkdirconf:
            try:
                os.mkdir(rdfpth)
            except Exception as fail:
                print("Mkdir failed: %s"%(fail))
        else:
            print("Mkdir cancelled")
        return
    ...
    #Create rdf file
    try:
        with open(rdfpth) as pth:
            pth.write("%s\n%s\n%s\n%s\n%s\nCreated by RDFWriter at %s"%(pth0, pth1, pth2, pth3, pth4, time.ctime()))
    except Exception as nfile:
        print("Process returned error %s.\nAttempt file creation at: %s..."%(nfile, rdfpth))
        try:
            with open(rdfpth, 'a') as pth:
                pth.write("%s\n%s\n%s\n%s\n%s\nCreated by RDFWriter at %s"%(pth0, pth1, pth2, pth3, pth4, time.ctime()))
                print("New file %s created."%(rdfpth))
        except Exception as nfilefail:
            print("Process returned error %s.\nCould not create new file.\nWill try mkdir...\nPlease remove file name and press [Create *rdf file] to cancel."%(nfilefail))
